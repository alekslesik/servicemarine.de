	...
	<?$this->SetViewTarget('section_bnr_content');?>
		<?\Aspro\Functions\CAsproAllcorp3::showBlockHtml(array(
			...
			'PARAMS' => array(
				...
				'BUTTONS' => array(
					array(
						//'TITLE' => Loc::getMessage('MORE_TEXT_BOTTOM'),
	...
	<?$this->SetViewTarget('PRODUCT_VIDEO_INFO');?>
		<div class="hidden_print">
			<div class="video_block">
				<div class="grid-list grid-list--gap-20 <?=($bOneVideo ? ' grid-list--items-1' : '')?>">
...
//					'QUESTION_BTN' => $bAskButton,
				...
				<?if(strlen($arResult['INCLUDE_CONTENT']) AND FALSE):?>
...
<div class="catalog-detail__top-info flexbox flexbox--direction-row flexbox--wrap-nowrap">
	...
	<div class="catalog-detail__gallery catalog-detail__gallery--<?=($arResult['GALLERY_SIZE'])?><?=($bGalleryThmbVertical ? ' catalog-detail__gallery--vertical' : '')?> swipeignore image-list__link">
					...
					<?foreach($arResult['GALLERY'] as $i => $arImage):?>
						...
						<div id="big-photo-<?=$i?>" class="catalog-detail__gallery__item catalog-detail__gallery__item--big">
							<a href="<?=$url?>" data-fancybox="gallery" class="catalog-detail__gallery__link popup_link fancy" title="<?=$title?>">
								<img class="catalog-detail__gallery__picture" src="<?=$arResult['DETAIL_PICTURE']['SRC']?>" alt="<?=$alt?>" title="<?=$title?>" />
							</a>
						</div>
					<?endforeach;?>
				<?else:?>
